...
// But below are the lines of code  that I'm using in the other sketch
...
//getADCrange
      if ( getADCRange != getADCRange() && getADCRange() > 0 ) {
        Serial.println("setting  adc");
        ADCRange = getADCRange();
        uint8_t error = MAX30105.setADCrange(ADCRange);
        if (error == 0){// Zero errors.
          Serial.println("PPG ADC Set.");
        }
      }
